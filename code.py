...
class TSPApp:
    ...
    def calculate_tour_cost(self, tour, graph_data):
        """Вычисляет общую стоимость маршрута, включая возврат к началу."""
        cost = 0
        for i in range(len(tour) - 1):
            if tour[i + 1] in graph_data[tour[i]]:
                cost += graph_data[tour[i]][tour[i + 1]]
            else:
                return float('inf')  
        if tour[0] in graph_data[tour[-1]]:
            cost += graph_data[tour[-1]][tour[0]]
        else:
            return float('inf')
        return cost
    ...
    def simulated_annealing(self, graph_data, use_cauchy, T0=1000, alpha=0.99, max_iterations=10000):
        ...
        for k in range(1, max_iterations + 1):
            ...
            new_cost = self.calculate_tour_cost(new_tour, graph_data)
